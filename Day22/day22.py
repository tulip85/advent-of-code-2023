...
def is_coord_below_occupied(coordinates, block_id):
    all_coordinates_temp = list(COORDINATE_REGISTRY.values())

    all_coordinates = []
    for t in all_coordinates_temp:
        all_coordinates += t

    for coord in COORDINATE_REGISTRY[block_id]:
...
def make_blocks_fall():
    ...
    while SORTING:
        ...
        for b_id in range(0, BLOCK_ID):
            if not is_coord_below_occupied(COORDINATE_REGISTRY[b_id], b_id):
...
RESULT1 = 0
for b_id in range(0, BLOCK_ID):
    CAN_BE_DISINTEGRATE = True
    for supporting in IS_SUPPORTING[b_id]:
        if len(SUPPORTED_BY[supporting]) == 1:
            CAN_BE_DISINTEGRATE = False

    if CAN_BE_DISINTEGRATE:
        RESULT1 += 1

print("Can be disintegrated", RESULT1)
